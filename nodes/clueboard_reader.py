...
class ClueBoardDetector:
    def __init__(self):
        ...
        model_path = os.path.join(os.path.dirname(__file__), "..", "reference", "CNNs", "ClueboardCNN.keras")
...
def prepare_for_nn(crops):
    crops = np.array(crops).astype("float32") / 255.0
    return crops.reshape(-1, 80, 54, 1)
